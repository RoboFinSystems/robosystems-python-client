...
This repo is public and the release publishes to PyPI in the same run, so the notes are world-readable immediately and are read by everyone upgrading. For any security-adjacent change:
...
- No `# RoboSystems Python SDK v<version>` heading, no release-statistics section, no links section, no generated-with footer. The workflow supplies all of those. Start at the first line of prose.
