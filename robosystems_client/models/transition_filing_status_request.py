...
class TransitionFilingStatusRequest:
  ...
  land cleanly.
      ...
      Attributes:
          ...
          target_status (str): Target lifecycle state: `under_review` (submit a draft for review), `draft` (send it back),
              `archived` (take a filed report off the current list; it stays a record) or `filed` (unarchive). Filing a draft
              goes through `file-report` so audit fields land cleanly.
  """
