...
class SecurityResponse:
  ...
  Attributes:
      ...
      security_type (str): Instrument family (e.g. `common_stock`, `preferred_stock`, `warrant`, `convertible_note`).
      terms (SecurityResponseTerms): Instrument-specific terms blob. Shape depends on `security_type` — see
          `CreateSecurityRequest.terms` for common keys.
      is_active (bool): `true` when the security is in active status; `false` after a soft-delete or deactivation.
      created_at (datetime.datetime): Row creation timestamp (UTC).
      updated_at (datetime.datetime): Last-modified timestamp (UTC).
      entity_id (None | str | Unset): ID of the issuing entity, when set.
      entity_name (None | str | Unset): Cached display name of the issuing entity, denormalized for list rendering.
          May lag the entity row's current name briefly.
      source_graph_id (None | str | Unset): Tenant graph this security is pre-associated to, when the issuer entity
          hasn't been promoted yet.
      security_subtype (None | str | Unset): Optional subtype refinement (e.g. `class_a`, `series_a`).
      authorized_shares (int | None | Unset): Total shares authorized for this class, when set.
      outstanding_shares (int | None | Unset): Shares currently issued and outstanding.
  """
