...
directly instead of pulling in a dedicated library like `gql`. Typed
response models are generated by ariadne-codegen (see
``robosystems_client/graphql/generated/``) from the checked-in schema
snapshot plus the query documents in
``robosystems_client/graphql/operations/``; facade methods send the
``*_GQL`` document constants through this transport and validate the raw
``data`` dict into the generated Pydantic models.
...
def keys_to_snake(value: Any) -> Any:
  ...
  Preserves scalar values, lists, and nested dicts. Exported utility for
  callers reshaping camelCase GraphQL JSON into snake_case dicts (the
  facades themselves now validate responses into the codegen models,
  which handle aliasing natively).
  """
