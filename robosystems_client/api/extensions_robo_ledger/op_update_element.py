...
def sync_detailed(
  ...
  *,
  ...
  """Update Element
   ...
   Update mutable fields on an element. `taxonomy_id` and `source` are immutable. Reparenting cascades
  path/depth to descendants.
  ...
  **Idempotency**: supply an `Idempotency-Key` header to make safe retries; replays within 24 hours
  ...
  Args:
      ...
      body (UpdateElementRequest): Update mutable fields on an element. `taxonomy_id` and
          `source` are
